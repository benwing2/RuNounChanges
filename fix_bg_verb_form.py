  ...
  for index, page in blib.cat_articles("Bulgarian verbs"):
    ...
    for t in parsed.filter_templates():
      if tname(t) == "bg-verb":
        verb = getparam(t, "1")
        if not verb:
          pagemsg("WARNING: Missing headword in verb: %s" % unicode(t))
          continue
        if bglib.needs_accents(verb):
          pagemsg("WARNING: Verb %s missing an accent: %s" % (verb, unicode(t)))
          continue
        ...
        if unaccented_verb in verbs_to_accents and verbs_to_accents[unaccented_verb] != verb:
          pagemsg("WARNING: Two different accents possible for %s: %s and %s: %s" % (
            unaccented_verb, verbs_to_accents[unaccented_verb], verb, unicode(t)))
...
def process_page(page, index, parsed):
    ...
    if tn == "bg-verb form of":
      ...
      if parttype == "vnform":
        if tname(headt) == "head":
          ...
          if not heads:
            pagemsg("WARNING: Something wrong, {{head|bg|verb form}} missing head=: %s" % unicode(headt))
            continue
        ...
        if tname(headt) == "head":
          ...
          if getparam(headt, "5"):
            pagemsg("WARNING: Too many participles attached to {{bg-part}}: %s" % unicode(headt))
            continue
          origheadt = unicode(headt)
          if getparam(headt, "4"):
            ...
            pagemsg("WARNING: Something wrong, no participle in {{bg-part}}: %s" % unicode(headt))
          pagemsg("Replaced %s with %s" % (origheadt, unicode(headt)))
          ...
          pagemsg("WARNING: Something wrong, unrecognized head template: %s" % unicode(headt))
          continue
  ...
  if vn_forms_to_add_header:
    for headt, inflt, gender in vn_forms_to_add_header:
      m = re.search("(=+)\n%s" % re.escape(unicode(headt)), text)
      if not m:
        pagemsg("WARNING: Something wrong, can't find head template %s in text" % unicode(headt))
        continue
      indents = m.group(1)
      headword = getparam(headt, "1")
      origtext = "# " + unicode(inflt)
