...
#    rename.py is free software: you can redistribute it and/or modify
...
def rename_pages(refrom, reto, refs, page_and_refs, cat, pages, pagefile,
  ...
  for index, page in pages:
    ...
    if filter_pages and not re.search(filter_pages, pagetitle):
      ...
      if verbose:
        pagemsg("Processing")
      new_pagetitle = re.sub(refrom, reto, pagetitle)
      this_comment = comment or (
          'renaming based on regex %s -> %s' % (refrom, reto))
      if new_pagetitle == pagetitle:
        ...
        if new_page.exists():
          ...
          try:
            page.move(new_pagetitle, reason=this_comment, movetalk=True, noredirect=True)
            errandpagemsg("Renamed to %s" % new_pagetitle)
          except pywikibot.PageRelatedError as error:
            errandpagemsg("Error moving to %s: %s" % (new_pagetitle, error))
        else:
          pagemsg("Would rename to %s (comment=%s)" % (new_pagetitle, this_comment))

pa = blib.init_argparser("Rename pages")
pa.add_argument("-f", "--from", help="From regex",
    metavar="FROM", dest="from_", required=True)
pa.add_argument("-t", "--to", help="To regex",
    required=True)
...
from_ = params.from_.decode("utf-8")
to = params.to.decode("utf-8")
...
rename_pages(from_, to, references, page_and_refs, category, pages, params.pagefile,
