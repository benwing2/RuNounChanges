...
stress_patterns["a"] = {
	nom_s="-", gen_s="-", dat_s="-", acc_s="-", ins_s="-", loc_s="-",
...
decls["fourth-n"] = function(base, stress)
	...
	add_decl(base, stress, "я", "я", "ю", "я", "ям", loc_sg)
	if base.plhard then
		add_decl(base, stress, nil, nil, nil, nil, nil, nil,
			"а", gen_pl_end_stressed and "ів" or "", "ам", "амі", "ах")
	else
		local gen_pl =
...
decls["t-n"] = function(base, stress)
	...
	add_decl(base, stress, v, v .. "ти", v .. "ті", v, v .. "м", v .. "ті",
		v .. "та", v .. "т", v .. "там", v .. "тамі", v .. "тах")
end
...
decls["adj"] = function(base, stress)
	...
	local adj_alternant_spec = require("Module:be-adjective").do_generate_forms({base.lemma .. propspec})
	...
	if inside ~= "" then
		...
		for i, dot_separated_group in ipairs(dot_separated_groups) do
			...
			elseif part == "inan" or part == "pr" or part == "anml" then
				...
				base.animacy = part
			elseif part == "ao" or part == "ae" or part == "yo" or part == "oy" then
				if base.valt then
					error("Can't specify vowel alternation indicator twice: '" .. inside .. "'")
				end
				base.valt = part
...
local function determine_stress_and_stems(base)
	...
	if base.stem then
		base.stem = com.add_monosyllabic_accent(base.stem)
	end
	if base.plstem then
		base.plstem = com.add_monosyllabic_accent(base.plstem)
	end
	for _, stress in ipairs(base.stresses) do
	...
	if alternant_multiword_spec.manual then
		...
		local function do_word_spec(base)
			...
			for _, stress in ipairs(base.stresses) do
				...
				local vowelalt
				if base.valt == "ae" then
					vowelalt = "а-е"
				elseif base.valt == "ao" then
					vowelalt = "а-о"
				elseif base.valt == "yo" then
					vowelalt = "ы-о"
				elseif base.valt == "oy" then
					vowelalt = "о-ы"
				end
				if vowelalt then
...
function export.catboiler(frame)
	...
	while true do
		...
		if not stem then
			-- check for e.g. 'Belarusian hard masculine accent-a nouns in -о'
			stem, gender, stress, pos, ending = rmatch(SUBPAGENAME, "^Belarusian (.-) ([a-z]+ine) accent%-(.-) (.*)s in %-([ое])$")
			if stem then
		...
		if not stem then
			-- check for e.g. 'Belarusian hard masculine nouns in -о'
			stem, gender, pos, ending = rmatch(SUBPAGENAME, "^Belarusian (.-) ([a-z]+ine) (.*)s in %-([ое])$")
			if stem then
				gender = gender .. " in -" .. ending
			end
		end
		...
		local altfrom, altto
		pos, altfrom, altto = rmatch(SUBPAGENAME, "^Belarusian (.*)s with (.*)%-(.*) alternation$")
		if altfrom then
