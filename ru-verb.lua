...
conjugations["4a1a"] = function(args, data)
	...
	-- iotation (т -> щ, not ч)
	parse_variants(data, args[1], {"23", "и", "щ", "past", "+p", "7", "жд"})
	local stem1, tr1 = nom.split_russian_tr(get_stressed_arg(args, 2))
...
local function guts_of_5(args, data)
	...
	if is5a then
		stem = get_stressed_arg(args, 2)
		-- обидеть, выстоять have different past tense and infinitive forms
		past_stem = get_opt_stressed_arg(args, 3) or stem .. "е"
	elseif is5b then
		stem = get_unstressed_arg(args, 2)
		past_stem = get_stressed_arg(args, 3)
	else
		stem = get_stressed_arg(args, 2)
		past_stem = get_stressed_arg(args, 3)
	end
...
local function guts_of_6a(args, data, vclass)
	...
	local inf_past_stem = get_opt_stressed_arg(args, 3) or
...
conjugations["8b"] = function(args, data)
	...
	local stem = data.past_stress == "b" and args[4] and getarg(args, 2) or
		get_stressed_arg(args, 2)
	local full_inf = get_stressed_arg(args, 3)
	local stressed_past_stem = args[4] and get_stressed_arg(args, 4) or stem
	no_stray_args(args, 4)
	forms["infinitive"] = full_inf
...
conjugations["irreg-дать"] = function(args, data)
	...
	parse_variants(data, args[1], {"past"}, prefix == "вы́" and "a(1)" or data.refl and "c''" or "c'")
	no_stray_args(args, 2)
...
conjugations["irreg-мочь"] = function(args, data)
	...
	-- no past_adv_part or past_adv_part_short
...
conjugations["irreg-лечь"] = function(args, data)
	...
	-- no pres parts (FIXME, what about pres_act_part?)
...
conjugations["irreg-клясть"] = function(args, data)
	...
	forms["pres_pasv_part"] = prefix .. "кляни́мый"
...
conjugations["irreg-стелить-стлать"] = function(args, data)
	...
	local infprefix = args[3] or ""
	local presprefix = args[4] or infprefix
	local stem = get_stressed_arg(args, 2)
	...
	forms["past_actv_part"] = infprefix .. stem .. "вший"
	forms["past_adv_part"] = infprefix .. stem .. "вши"
	forms["past_adv_part_short"] = infprefix  .. stem .. "в"

	if prefix == "вы́" then
		forms["pres_pasv_part"] = presprefix .. "стелимый"
		forms["pres_actv_part"] = presprefix .. "стелющий"
		forms["pres_adv_part"] = presprefix .. "стеля"

		forms["impr_sg"] = presprefix .. "стели"
		forms["impr_pl"] = presprefix .. "стелите"
		present_je_c(forms, presprefix .. "стел")
	else
		forms["pres_pasv_part"] = presprefix .. "стели́мый"
		forms["pres_actv_part"] = presprefix .. "сте́лющий"
		forms["pres_adv_part"] = presprefix .. "стеля́"
...
conjugations["irreg-внимать"] = function(args, data)
	...
	forms["infinitive"] = "внима́ть"

	-- handle внемл- forms
	append_participles(forms, "вне́мл", nil,
		"ющий", "емый", {"я", "я́"}, {}, {}, {})
	-- handle внима́- forms
	append_participles(forms, "внима́", nil,
		"ющий", "емый", "я", "вший", "вши", "в")
	set_imper(forms, "", nil, {"вне́мли", "внемли́", "внима́й"},
		{"вне́млите", "внемли́те", "внима́йте"})
	present_je_a(forms, "вне́мл")
	-- Both вне́млю and внемлю́ are possible
	append_pres_futr(forms, "внемл", nil, "ю́", {}, {}, {}, {}, {})
	present_je_a(forms, "внима́")
	set_past(forms, "внима́л", nil, "", "а", "о", "и")
	prepend_prefix(forms, prefix)
