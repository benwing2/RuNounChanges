...
-- accents in them differently. Specifically we need to be able to convert a generated form into both linkless
-- accented and linkless unaccented text (where 'unaccented' means containing only the accents in the normal
-- written form instead of extra stress/vowel-quality accents). Linkless accented text can be generated by calling
-- remove_links() in [[Module:links]] but for unaccented text it's a bit trickier, both because of single-syllable
-- accented words (accented 'dà' may map to unaccented 'da' or 'dà') and because of foreign words with non-final
-- accents in the 'unaccented' form, e.g. [[bohémien]]. For verb forms that we generate ourselves, the left side
-- of a two-part link is always the unaccented form (or we can ensure this is the case). For two-part links from
-- the user, however, the left side may be the lemma and the right side an accented non-lemma form, so we have
-- to convert the accented form to unaccented by removing non-final accents unless the left side has an accent
-- in it, in which case we leave all accents.
...
function export.convert_to_unaccented(form)
	return rsub(form, "%[%[([^%[%]]+)%]%]",
		function(linktext)
			local left, right = linktext:match("^%[%[(.*)|(.*)%]%]$")
			if left then
				-- a two-part link
