...
local function add_present_indic(base)
	...
	if base.conj == "are" then
		s2 = rfind(base.stems.pres_stressed, "[cg]$") and "hi" or "i"
		p1 = rfind(base.stems.pres_unstressed, "[cg]$") and "hiàmo" or "iàmo"
		s2, p2, p3 = "a", "àte", "ano"
	elseif base.conj == "ere" then
		s2, s3, p1, p2, p3 = "i", "e", "iàmo", "éte", "ono"
	elseif base.conj == "ire" then
		s2, s3, p1, p2, p3 = "i", "e", "iàmo", "ìte", "ono"
	else
		error("Internal error: Unrecognized conjugation " .. base.conj)
	end

	addit("1s", base.stems.pres1, s1)
...
local function add_present_subj(base)
	...
	local s123, p1, p2, p3
	if base.conj == "are" then
		if rfind(base.stems.pres_sub_stressed, "[cg]$") then
			s123, p3 = "hi", "hino"
		else
			s123, p3 = "i", "ino"
		end
		if rfind(base.stems.pres_sub_unstressed, "[cg]$") then
			p1, p2 = "hiàmo", "hiàte"
		else
			p1, p2 = "iàmo", "iàte"
		end
	else
		s123, p1, p2, p3 = "a", "iàmo", "iàte", "ano"
	end

	addit("1s", base.stems.pres_sub_stressed, s123)
	addit("2s", base.stems.pres_sub_stressed, s123)
	addit("3s", base.stems.pres_sub_stressed, s123)
...
local function add_non_present(base)
	...
	add_tense("impf", stems.impf, "o", "i", "a", "àmo", "àte", "ano")

	for _, pret in ipairs(stems.pret_forms) do
		if pret.ending == "ài" then
			add_tense("pret", pret.stem, "ài", "àsti", "ò", "àmmo", "àste", "àrono")
		elseif pret.ending == "éi" then
			add_tense("pret", pret.stem, "éi", "ésti", "é", "émmo", "éste", "érono")
		elseif pret.ending == "ètti" then
			add_tense("pret", pret.stem, "ètti", "ésti", "ètte", "émmo", "éste", "èttero")
		elseif pret.ending == "ìi" then
			add_tense("pret", pret.stem, "ìi", "ìsti", "ì", "ìmmo", "ìste", "ìrono")
		elseif pret.ending == "i" then
			local conj_vowel
			if base.conj == "are" then
				conj_vowel = "à"
			elseif base.conj == "ere" then
			...
			add_tense("pret", pret.irreg_stem, "i", {}, "e", {}, {}, "ero")
			add_tense("pret", pret.stem, {}, conj_vowel .. "sti", {}, conj_vowel .. "mmo", conj_vowel .. "ste", {})
		else
			error("Internal error: Unrecognized preterite ending " .. pret.ending)
		end
	end
