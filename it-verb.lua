...
-- Otherwise:
   1. If imp: given, this specifies the 2s form; otherwise the 2s form is generated from the infinitive stem (which
      may be explicitly given in cases like [[condurre]]) by replacing the final vowel with -i for -ere verbs,
	  otherwise keeping the final vowel.
...
Comment: Generally uses the same form as {{it-verb}}. Automatically recognizes the ending (-are/-ere/-ire) and
conjugates appropriately. In the preterite, forms ending in consonant + unstressed i are recognized and automatically
generate a conjugation of the form mìsi, mettésti, mìse, mettémmo, mettéste, mìsero.
...
{{it-conj|fare<a/-,féci,fàtto.
  ...
  imp:fà*:fài:fà'
>}}
; subjunctive 123s/p pòssa/pòssano generated from pres1s, 1p/2p possiàmo/possiàte generated from pres1p
{{it-conj|potere<a:e[as an auxiliary, with main verbs taking ''essere'']/-,potéi:potètti[less common].
  ...
  noimp
>}}
; imperative 2p dìte generated pres2p
{{it-conj|dire<a/-,dìssi,détto.
  stem:dìce.
  pres2p:dìte.
  imp:dì':*dì*
>}}
; subjunctive 1p/2p dobbiàmo/dobbiàte generated from pres1p
...
{{it-conj|vedere<a/é,vìdi,vìsto:vedùto[less popular].
  ...
  fut:vedrò
>}}
; subjunctive 123s/3p vènga/vèngano generated from pres1s, 1p/2p veniàmo/veniàte generated from pres1p
; imperative 2s vièni generated from pres2s, 2p venìte generated from pres2p
...
{{it-conj|sapere<a:e[as an auxiliary, with main verbs taking ''essere'']/-,sèppi.
  ...
  fut:saprò.
  suball:sàppia.
  impall:sàppi:sappiàte
>}}
{{it-conj|togliere<a\ò\tòlgo,tòlsi,tòlto.fut:+,torrèi[literary].presp:+>}}
...
local function combine_stem_ending(base, slot, stem, ending)
	if ending:find("^i") then
		if base.conj == "are" and stem:find("[cg]$") then
			stem = stem .. "h"
		end
		stem = stem:gsub("i$", "")
	end
...
local function add_present_indic(base)
	...
	if base.irregrow_forms.presrow then
		add_irregrow(base, prespref, "presrow")
		return
	end
	...
	local pres_1s_stem = iut.map_forms(base.stems.pres_form, function(form)
		if not form:find("o$") then
			error("presrow: must be given in order to generate the singular present subjunctive "
	...
	addit("2p", base.verb.stem, base.conj == "are" and "àte" or base.conj == "ire" and "ìte" or "éte")
end
...
local function add_present_subj(base)
	...
	if base.irregrow_forms.subrow then
		add_irregrow(base, subpref, "subrow")
		return
	end
	...
	if base.irregrow_forms.sub then
		...
		local pres_1s_stem = iut.map_forms(base.forms.pres_1s, function(form)
			if not form:find("o$") then
				error("sub:, subrow: or sub123s: must be given in order to generate the singular present subjunctive "
					.. "because first-person singular present indicative '" .. form .. "' does not end in -o")
			end
			return rsub(form, "o$", "")
		end)
	...
	insit("2p", iut.map_forms(base.forms.pres_1p, function(form)
		if not form:find("mo$") then
			error("subrow: or sub2p: must be given in order to generate the second-person plural present subjunctive "
