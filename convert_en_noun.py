    ...
    if tn == "en-noun":
      ...
      if not saw_plural and not no_default_plural:
        ...
        if plural == default_plural:
          ...
          pagemsg("WARNING: Saw {{en-noun}} without explicit plural and new default plural %s different from old default plural %s: %s" %
            (default_plural, plural, unicode(t)))
        t.add(newparam, plparam)
        plurals.append(plparam)
      if not no_default_plural and plurals == ["+"]:
        if has_plqual:
          pagemsg("WARNING: Plurals are only '+' but plqual= present, not removing: %s" % unicode(t))
        else:
          if getparam(t, "1") == "~":
            firstparam = 2
          else:
      ...
      if unicode(t) != origt:
        ...
        if plurals_with_s_and_es == orig_plurals:
          pagemsg("No changes if '+' is replaced with 's' or 'es' as appropriate: orig=%s, new=%s" % (origt, unicode(t)))
        else:
          num_would_save_even_with_s_or_es += 1
        if plurals_with_s == orig_plurals:
          pagemsg("No changes if '+' is replaced with 's' as appropriate: orig=%s, new=%s" % (origt, unicode(t)))
        else:
