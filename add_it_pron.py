...
def get_items(lines):
  for line in lines:
    m = re.search("^Page ([0-9]*) (.*): <respelling> *(.*?) *<end>")
    if not m:
      msg("WARNING: Unrecognized line: %s" % line)
    else:
      yield m.groups()

for _, (index, pagetitle, spec) in blib.iter_items(get_lines(lines), start, end, get_name=lambda x:x[1], get_index=lambda x:x[0]):
  page = pywikibot.Page(site, pagetitle)
  if not page.exists():
    ...
    blib.do_edit(page, index, do_process_page,
        save=program_args.save, verbose=program_args.verbose,
        diff=program_args.diff)
