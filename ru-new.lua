...
local function do_verb(args)
	...
	local defn = args[3]
	local aspect = args[4]
	local corverbs = args[5]
	local conj = {}
	for i = 6, 20 do
		table.insert(conj, args[i])
	end
	local translit
	if rfind(verb, "//") then
		local bases = rsplit(verb, "//")
		if #bases ~= 2 then
			error("Verb with manual translit should have only one //: " .. verb)
		verb, translit = bases[1], bases[2]
	end
	subs.pron = "* {{ru-IPA|" .. verb .. "}}\n"
	verb = rsub(verb, "[" .. GR .. CFLEX .. DOTABOVE .. DOTBELOW .. "]", "")
	subs.verb = verb
	if not rfind(verb, "ть$") and not rfind(verb, "ться$") and not rfind(verb, "ти́$") and
		not rfind(verb, "ти́сь$") and not rfind(verb, "чь$") and not rfind(verb, "чься$") then
		error("Lemma " .. verb .. " doesn't look like a verb infinitive")
	end
	check_stress(verb)
	subs.tr = translit and "|tr=" .. translit or ""
	local isrefl = rfind(verb, "с[яь]$")
	subs.headword_aspect = rsub(aspect, "%-.*", "")
	...
	subs.corverb = ""
	local corverbno = 1
	for _, corverb in ipairs(corverbs) do
		...
		subs.corverb = subs.corverb + "|" + ((subs.headword_aspect == "pf" or rfind(corverb, "^%*")) and "impf" or "pf") +
			...
			rsub(corverb, "^%*", ""
		corverbno = corverbno + 1
	local verbbase = rsub(verb, "с[яь]$", "")
	local trverbbase = translit and (rfind(translit, "sja$") and rsub(translit, "sja$", "") or
		rsub(translit, "sʹ$", ""))
	subs.passive = args.e == "r" and "# {{passive of|lang=ru|" .. verbbase .. (trverbbase and "|tr=" .. trverbbase or "") .. "}}\n" or ""

	local reflsuf = isrefl and "-refl" or ""
	local function order_aspect(aspect)
		return rsub(aspect, "-impers-refl", "-refl-impers")
	end
	local conjargs = generate_verb_args(conj, verbbase, trverbbase)
	local conjstr = table.concat(conjargs, "|")
	if rfind(aspect, "^both") then
		local pfaspect = rsub(aspect, "^both", "pf")
		local impfaspect = rsub(aspect, "^both", "impf")
		subsub = {
			impfaspect = order_aspect(impfaspect .. reflsuf),
			pfaspect = order_aspect(pfaspect .. reflsuf),
			conj = conjstr,
		}
...
{\op}{\op}ru-conj|{impfaspect}{conj}{\cl}{\cl}]===], subsub)
		...
		sub.conj = "{{ru-conj|" .. order_aspect(aspect .. reflsuf) .. "|" .. conjstr .. "}}"
	end
