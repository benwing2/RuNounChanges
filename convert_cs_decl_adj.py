  ...
  for t in parsed.filter_templates():
    ...
    if tn == "cs-adj":
      ...
      if constructed_pagename != pagetitle:
        pagemsg("WARNING: Weird params in %s, don't concatenate to pagename" % unicode(t))
        continue
      rmparam(t, "3")
      rmparam(t, "2")
      rmparam(t, "1")
      ...
      if pagetitle != par1 + u"í":
        pagemsg("WARNING: Weird 1=%s in %s" % (par1, unicode(t)))
        continue
      rmparam(t, "1")
      blib.set_template_name(t, "cs-adecl")
      ...
      if par2:
        if par2 != "ma":
          pagemsg("WARNING: Weird 2=%s in %s" % (par2, unicode(t)))
          continue
        par3 = getp("3")
        if par1.endswith("sk"):
          pal2 = par1[:-2] + u"št"
        ...
        if par3 != should_par3:
          pagemsg("WARNING: 3=%s should equal %s but doesn't: %s" % (par3, should_par3, unicode(t)))
          continue
      rmparam(t, "3")
      rmparam(t, "2")
      rmparam(t, "1")
