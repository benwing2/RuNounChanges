...
local function format_glosses(data)
	...
	elseif has_gloss then
		...
		for i, gloss in ipairs(data.gloss) do
			...
			data.gloss[i] = link_with_qualifiers(gloss,
				data.g == "f" and not gloss.gloss_is_gendered and "[[female]] " or nil)
		end
		ins(table.concat(data.t, ", "))
		ins(" (")
	end
...
function export.format_demonym_adj(data)
	...
	if lang_is_en then
		if has_gloss then
			local glosstext
			if #data.t == 1 then
				glosstext = ("an English gloss '%s'"):format(data.t[1].term)
			else
				for i, term in ipairs(data.t) do
					data.t[i] = "'" .. term.term .. "'"
				end
				glosstext = ("English glosses %s"):format(table.concat(data.t, ", "))
			end
			error(("Can't specify %s for the term when the language is already English"):format(glosstext))
		end
		if not has_femeq and not data.nocap then
	...
	for i, part in ipairs(data.parts) do
		...
		parts[i] = link_with_qualifiers(part)
	end

	if #parts == 1 then
		ins(parts[1])
	else
		ins(require("Module:table").serialCommaJoin(parts, {conj = "or"}))
	end
