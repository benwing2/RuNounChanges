...
from clean_label_module import LabelData
...
def process_text_on_page(index, pagename, text):
  ...
  for labelobj in regional_labelobjs_by_lang[langcode]:
    ...
    def comment_out(labelobj, dupmsg):
      new_lines = clean_label_module.output_labels([labelobj])
  ...
  new_lines = clean_label_module.output_labels(lang_specific_label_data.labels_seen)
  text = "\n".join(new_lines)
  text = text.replace("\n\n\n", "\n\n")
  ...
  if num_incorporated_regional_labelobjs > 0:
    ...
    notes = ["move %s label(s) (%s) from [[Module:labels/data/regional]] to lang-specific labels data module" % (
             num_incorporated_regional_labelobjs, " + ".join(label_msg_parts))]
  else:
...
if regional_label_data is None:
  ...
  for labelobj in regional_label_data.labels_seen:
    if isinstance(labelobj, LabelData):
      if args.categories:
        ...
        langs_for_label = labelobj.fields.langs
      else:
      ...
      if not langs_for_label:
        ...
        keep = True
      elif len(labelobj.aliases.cats) > 1 and len(langs_for_label) > 1:
        pagemsg("WARNING: Not removing regional label '%s' with %s aliases %s > 1 and %s languages %s > 1" % (
          labelobj.label, len(labelobj.aliases.cats), ",".join(labelobj.aliases.cats),
      ...
      if keep:
        ...
        pagemsg("Removing label %s from [[Module:labels/data/regional]], moved to lang-specific modules" %
                labelobj.label)
  ...
  for index, (lang, labelobjs) in enumerate(sorted(list(regional_labelobjs_by_lang.items()), key=lambda x: x[0])):
    ...
    if lang not in existing_langs_seen:
      ...
      new_lang_specific_lines = clean_label_module.output_labels(labelobjs_with_text)
      blib.do_handle_stdin_retval(
        args, ("\n".join(new_lang_specific_lines),
               "move %s label(s) from [[Module:labels/data/regional]] to new lang-specific module" %
               len(labelobjs)),
