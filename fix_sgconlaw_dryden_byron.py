...
# Move text outside of {{RQ:Dryden Meta}} and {{RQ:Byron Harold}} inside, with some renaming of
...
def process_text_on_page(index, pagename, text):
  ...
  def replace_dryden_meta(m):
    book, text = m.groups()
    book = arabic_to_roman(book)
    if not book:
      return m.group(0)
    text = re.sub(r"\s*<br */?>\s*", " / ", text)
    notes.append("reformat {{RQ:Dryden Meta}} into {{RQ:Dryden Metamorphoses}}")
    return "{{RQ:Dryden Metamorphoses|book=%s|passage=%s}}\n" % (book, text)

  curtext = re.sub(r"\{\{RQ:Dryden Meta\|([^|{}]*?)\}\}\n#+\*: (.*?)\n",
      replace_dryden_meta, curtext)

  def replace_dryden_meta_no_book(m):
    (text,) = m.groups()
    text = re.sub(r"\s*<br */?>\s*", " / ", text)
    notes.append("reformat {{RQ:Dryden Meta}} into {{RQ:Dryden Metamorphoses}}")
    return "{{RQ:Dryden Metamorphoses|passage=%s}}\n" % text

  curtext = re.sub(r"\{\{RQ:Dryden Meta\}\}\n#+\*: (.*?)\n",
      replace_dryden_meta_no_book, curtext)

  def replace_byron_harold(m):
    canto, stanza, text = m.groups()
    canto = arabic_to_roman(canto)
    if not canto:
      return m.group(0)
    stanza = arabic_to_roman(stanza)
    if not stanza:
      return m.group(0)
    text = re.sub(r"\s*<br */?>\s*", " / ", text)
    notes.append("reformat {{RQ:Byron Harold}} into {{RQ:Byron Childe Harold}}")
    return "{{RQ:Byron Childe Harold|canto=%s|stanza=%s|passage=%s}}\n" % (canto, stanza, text)

  curtext = re.sub(r"\{\{RQ:Byron Harold\|([^|{}]*?)\|([^|{}]*?)\}\}\n#+\*: (.*?)\n",
      replace_byron_harold, curtext)

  def replace_byron_harold_no_stanza(m):
    canto, text = m.groups()
    canto = arabic_to_roman(canto)
    if not canto:
      return m.group(0)
    text = re.sub(r"\s*<br */?>\s*", " / ", text)
    notes.append("reformat {{RQ:Byron Harold}} into {{RQ:Byron Childe Harold}}")
    return "{{RQ:Byron Childe Harold|canto=%s|passage=%s}}\n" % (canto, text)

  curtext = re.sub(r"\{\{RQ:Byron Harold\|([^|{}]*?)\}\}\n#+\*: (.*?)\n",
      replace_byron_harold_no_stanza, curtext)

  def replace_byron_harold_no_canto_stanza(m):
    (text,) = m.groups()
    text = re.sub(r"\s*<br */?>\s*", " / ", text)
    notes.append("reformat {{RQ:Byron Harold}} into {{RQ:Byron Childe Harold}}")
    return "{{RQ:Byron Childe Harold|passage=%s}}\n" % text

  curtext = re.sub(r"\{\{RQ:Byron Harold\}\}\n#+\*: (.*?)\n",
      replace_byron_harold_no_canto_stanza, curtext)

  return curtext.rstrip("\n"), notes

parser = blib.create_argparser("Reformat {{RQ:Dryden Meta}} and {{RQ:Byron Harold}}", include_pagefile=True,
    include_stdin=True)
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(args, start, end, process_text_on_page,
    default_refs=["Template:RQ:Dryden Meta", "Template:RQ:Byron Harold"], edit=True, stdin=True)
