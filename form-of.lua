...
`.lemmas`: List of objects describing the lemma(s) of which the term in question is a non-lemma form. These are passed
		   ...
		   Alternatively, `.lemmas` can be a string, which is displayed directly.
...
function export.format_form_of(data)
	...
	if data.lemmas then
		if type(data.lemmas) == "string" then
			table.insert(parts, wrap_in_span(data.lemmas, lemma_classes))
		else
	...
	if data.enclitics and #data.enclitics > 0 then
		...
		table.insert(parts, " (")
		table.insert(parts, wrap_in_span("with enclitic" .. (#data.enclitics > 1 and "s" or "") .. " ", text_classes))
		table.insert(parts, m_table.serialCommaJoin(formatted_terms))
		table.insert(parts, ")")
		table.insert(parts, "<span class='" .. text_classes .. "'>")
	end
...
function export.lookup_shortcut(tag, lang, do_track)
	...
	if langcode and langs_with_lang_specific_tags[langcode] then
		local langdata = mw.loadData(form_of_lang_data_module_prefix .. langcode)
	...
	if not expansion then
		local m_data = mw.loadData(form_of_data_module)
	...
	if not expansion then
		local m_data2 = mw.loadData(form_of_data2_module)
...
local function fetch_lang_categories_and_labels(lang, tags, POS, already_normalized)
	local m_cats = mw.loadData(form_of_cats_module)
	local categories = {}
	local labels = {}
...
function export.fetch_lang_categories(lang, tags, lemmas, POS)
	...
	local tag_sets = export.split_tags_into_tag_sets(tags)
	for _, tag_set in ipairs(tag_sets) do
		local this_categories, this_labels = fetch_lang_categories_and_labels(lang, tag_set, POS)
...
`.lemmas`: (REQUIRED) List of objects describing the lemma(s) of which the term in question is a non-lemma form. These
		   are passed directly to full_link() in [[Module:links]]. Each object should have at minimum a `.lang` field
		   containing the language of the lemma and a `.term` field containing the lemma itself. Each object is
		   formatted using full_link() and then if there are more than one, they are joined using serialCommaJoin() in
		   [[Module:table]]. Alternatively, `.lemmas` can be a string, which is displayed directly.
...
Two values are returned: the formatted text and a list of tag-set-related categories generated by the specs in
[[Module:form of/cats]]. Normally, the formatted text includes the formatted categories appended to it, so you don't
need to do this yourself with the returned categories. To suppress this, set data.no_format_categories = true.
...
function export.tagged_inflections(data)
	...
	local function format_labels(labels, notext)
		if labels and #labels > 0 then
			return require("Module:labels").show_labels(labels, data.lang, nil, nil, nil, nil, data.nocat) ..
				(notext and "" or " ")
		else
			return ""
		end
	...
	if not data.no_format_categories and #categories > 0 then
...
function export.dump_form_of_data(frame)
	local data = {
		data = require(form_of_data_module),
		data2 = require(form_of_data2_module)
	}
