...
def process_page(index, pagename, text):
  ...
  for t in parsed.filter_templates():
    tn = tname(t)
    if tn == "bg-noun" or tn == "bg-proper noun":
      headword = getparam(t, "1")
    if tn.startswith("bg-noun-"):
      origt = unicode(t)
      if not headword:
        pagemsg("WARNING: Saw %s without {{bg-noun}} headword" % origt)
        continue
      del t.params[:]
      t.add("1", "%s<>" % headword)
      blib.set_template_name(t, "bg-ndecl")
...
parser = blib.create_argparser("Use {{bg-ndecl}} for Bulgarian declensions")
