    ...
    if tn in ["cs-ndecl"]:
      if decl_template:
        pagemsg("WARNING: Multiple cs-ndecl templates %s and %s without intervening headword template, skipping" %
          (unicode(decl_template), unicode(t)))
        return
      decl_template = t
      arg1 = getparam(t, "1")
      if arg1.startswith("m.an"):
        ...
        if ".an" in arg1:
          pagemsg("WARNING: Misplaced animacy spec: %s" % unicode(t))
          return
        gender = "m-in"
        ...
        pagemsg("WARNING: Unable to extract gender from noun declension, skipping: %s" % unicode(t))
        return
      if ".pl" in arg1:
        gender += "-p"
      if headword_template is None:
        pagemsg("WARNING: Saw declension template %s without preceding headword template: %s" % unicode(t))
        return
      ...
      if check_gender:
        if headword_genders != [gender]:
          pagemsg("WARNING: Headword gender(s) %s disagree with declension gender %s, skipping: %s" % (",".join(headword_genders), gender, unicode(t)))
          return
      headword_template = None
