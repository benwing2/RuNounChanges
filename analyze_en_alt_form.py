...
for lineno, line in blib.iter_items_from_file(args.direcfile, start, end):
  ...
  transformations = [
    ...
    (lambda x: re.sub(r"\b([Aa])rse", r"\1ss", x), "arse -> ass", Pondian("arse-form", "ass-form", pronun_different=True)),
    ...
    (lambda x: re.sub("([Ss])torey", r"\1tory", x), "storey -> story", Pondian("storey-form", "story-form")),
    # sulphur, but also sulphide, sulphite, sulphate etc.
    (lambda x: re.sub("([Ss])ulph", r"\1ulf", x), "sulph -> sulf", Pondian("sulph-form", "sulf-form")),
    (lambda x: re.sub("([Aa])luminium", r"\1luminum", x), "aluminium -> aluminum", Pondian("aluminium-form", "aluminum-form", pronun_different=True)),
    (lambda x: re.sub("([Nn])eurone\b", r"\1euron", x), "neurone -> neuron", Pondian("neurone-form", "neuron-form", pronun_different=True)),
    (lambda x: re.sub("([Dd])raught", r"\1raft", x), "draught -> draft", Pondian("draught-form", "draft-form")),
    (lambda x: re.sub("([Cc])heque", r"\1heck", x), "cheque -> check", Pondian("cheque-form", "check-form")),
    (lambda x: re.sub("([Ss])ceptic", r"\1keptic", x), "sceptic -> skeptic", Pondian("sceptic-form", "skeptic-form")),
    (lambda x: re.sub("([Aa])rtefact", r"\1rtifact", x), "artefact -> artifact", Pondian("artefact-form", "artifact-form")),
    # programme, spectogramme, decagramme, etc.
    (lambda x: re.sub("([Gg])ramme", r"\1ram", x), "gramme -> gram", Pondian("gramme-form", "gram-form")),
    # judgement, lodgement, abridgement
    (lambda x: x.replace("dgement", "dgment"), "dgement -> dgment", Pondian("dgement-form", "dgment-form")),
    (lambda x: re.sub("([Pp])ractise", r"\1ractice", x), "practise -> practice", Pondian("practise-form", "practice-form")),
    (lambda x: re.sub("([Mm])anoeuvre", r"\1aneuver", x), "manoeuvre -> maneuver", Pondian("manoeuvre-form", "maneuver-form")),
    (lambda x: re.sub("([Dd]ef|[Oo]ff|[Ll]ic|[Pp]ret)ence", r"\1ense", x),
      "defence/offence/licence/pretence -> defense/offense/license/pretense", Pondian("ence-form", "ense-form")),
    (lambda x: re.sub(r"([Cc]ol|[FfSs]av|[Ll]ab|[Vv]ap|[Oo]d|[Ss]ucc|[Hh]arb|[Aa]r[bm]|[TtHhRr]um|[Rr]ig|[Bb]ehavi|[Ee]nam|[Ee]ndeav|[Nn]eighb|[Cc]lam|[Dd]emean|[Hh]on|[Rr]anc|[Ss]avi|[Pp]arl|[Gg]lam)our", r"\1or", x),
    ...
    (lambda x: re.sub("(ae|oe|æ|œ)", "e", x), "æ/œ/ae/oe -> e", Pondian("ae-oe-form", "e-not-ae-oe-form")),
