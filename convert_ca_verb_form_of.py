    ...
    for t in parsed.filter_templates():
      ...
      if tn == "ca-conj":
        ...
        if arg1 not in conjs:
          conjs.append(arg1)
    if len(conjs) == 0:
      if re.search(r"==\s*Catalan\s*==", conjtext):
        warning = "WARNING: Infinitive page exists and has a ==Catalan== section but has no conjugations"
      else:
        warning = "WARNING: Infinitive page exists but does not have a ==Catalan== section"
    else:
      if len(conjs) > 1:
...
def process_text_on_page(index, pagetitle, pagetext):
  ...
  def do_sectext(sectext, do_infl_of):
    ...
    for k in range(1, len(chunks), 2):
      ...
      if not re.search(r"\A((?:# \{\{%s\|.*\}\}\n)+)\Z" % tname_re, verb_form_chunk):
        m = re.search(r"\A# \{\{(%s)\|.*\}\}(.*)\n\Z" % tname_re, verb_form_chunk)
        if m:
          pagemsg("WARNING: Extraneous text after {{%s}}, adding after new {{ca-verb form of}}: <%s>"
          ...
          if do_infl_of:
            ...
            possible_templates = "{{ca-verb form of}}"
      ...
      for t in parsed.filter_templates():
        ...
        if tn == "ca-verb form of":
          is_old = False
          oldparams = ["g", "p", "n", "t", "m", "nocap", "nodot"]
          for oldparam in oldparams:
            if t.has(oldparam):
              is_old = True
              break
          if not is_old:
            pagemsg("Saw new-style {{ca-verb form of}}, skipping: %s" % origt)
            must_continue = True
            break
          inf = getp("1")
        elif tn in ["inflection of", "infl of"] and getp("1") == "ca":
        ...
        for conj in conjs:
          blib.set_template_name(t, "ca-verb form of")
          del t.params[:]
          t.add("1", conj)
          newtemp = str(t)
        ...
        old_template_desc = tn == "ca-verb form of" and "old-style {{ca-verb form of}}" or "{{%s}}" % tn
        if len(expansions) == 0:
        ...
        notes.append("replace %s conjugation(s) for infinitive [[%s]] with {{ca-verb form of|%s}}" % (old_template_desc, inf, conj))
        del t.params[:]
        t.add("1", conj)
        newtemp = str(t)
  ...
  # First do {{ca-verb form of}} and {{ca-verb-form-of}}.
  pagetext = do_sectext(pagetext, do_infl_of=False)

  # Then do {{inflection of}}. Do this second; if we do it first, the resulting new-style {{ca-verb form of}}
  # triggers a needless warning.
  subsections = re.split("(^==+[^=\n]+==+\n)", pagetext, 0, re.M)
  for k in range(2, len(subsections), 2):
    if "=Verb=" in subsections[k - 1] and re.search(r"\{\{head\|ca\|verb form[|}]", subsections[k]):
      parsed = blib.parse_text(subsections[k])
      must_continue = False
      for t in parsed.filter_templates():
        tn = tname(t)
        if tn in ["ca-verb", "ca-conj"]:
          pagemsg("WARNING: Saw verb form along with verb, skipping: %s" % (str(t)))
          must_continue = True
          break
        if tn == "head" and getparam(t, "1") != "ca":
...
parser = blib.create_argparser("Convert {{ca-verb form of}} to new format", include_pagefile=True, include_stdin=True)
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(args, start, end, process_text_on_page, edit=True, stdin=True,
    default_refs=["Template:ca-verb form of"], skip_ignorable_pages=True)
